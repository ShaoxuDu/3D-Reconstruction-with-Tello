...
class TelloUI:
    ...
    def __init__(self, tello, outputpath):
        ...
        self.degree = 6  # default degree for 'cw' or 'ccw' cmd
    ...
    def openCmdWindow(self):
        ...
        """
        start_command = ['command', 'takeoff', 'forward 115', 'cw 180']
        loop_command = ['ccw 5', 'right 20', 'ccw 5']
        end_command = ['forward 115', 'cw 180', 'land']

        command = start_command + 36 * loop_command + end_command
        print(command)
        self.tello.set_speed(3)
        for i in range(len(command)):
            c = command[i]

            while True:
                time.sleep(3)
                res = self.tello.send_command(c)
                print(res)
                if res == 'ok':
